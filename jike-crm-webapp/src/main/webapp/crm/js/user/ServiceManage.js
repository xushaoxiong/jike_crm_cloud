...
//添加关联学校
	$('.newlist').click(function(){
		$('.R-wap').load('user/ServiceManageCreat.html',function(){
			breadnav(Fht,netht);
		});
...
function list(List){
	...
	$.each(List, function(i,item) {
		Html+='<tr>';
			Html+='<td>'+(i+1)+'</td>';
			Html+='<td class="serviceLaderName" leaderid="'+item.leaderId+'">'+item.leaderName+'</td>';	
...
//	//搜索
	...
	function editBuiness(_this){
		var BymanagePArry=[];
		var leaderId=_this.parents('tr').find('.serviceLaderName').attr('leaderid');
		var leaderhtml=_this.parents('tr').find('.serviceLaderName').html();
		...
		$('.R-wap').load('user/ServiceManageEdit.html',function(){
	...
	function delBuiness(_this){
		...
		$('.serverName').html($('.serviceLaderName[leaderid="'+leaderId+'"]').html())
	}
	$('.delConfirm').click(function(){
		...
		$ajax('post','user/deleteServiceLeader',delJ,function succF(jo){
			$('.serviceLaderName[leaderid="'+leaderId+'"]').parent().remove();
		},function errF(jo){
			pub.Alt(jo.message,false);
		})
	})